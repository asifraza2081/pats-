...
@section('page-title', 'Edit Job Post: ' . $job->title)

@section('page-actions')
<a href="{{ route('admin.projects.show', $job->project_id) }}" class="btn btn-outline-secondary">
...
<div class="row justify-content-center">
    <div class="col-lg-10">
        <form method="POST" action="{{ route('admin.jobs.update', $job) }}" class="card shadow-sm border-0">
            @csrf @method('PUT')
            
            <div class="card-header border-0 pb-1 pt-3">
                <h3 class="card-title fw-bold text-primary">Job Post Setup: {{ $job->project->name }}</h3>
            </div>
                ...
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <select name="min_degree_level" class="form-select tom-select">
                            ...
                            @foreach([1=>'Matric (10 Years)', 2=>'Intermediate (12 Years)', 3=>"Bachelor's (14/16 Years)", 4=>"Master's (16/18 Years)", 5=>'M.Phil (18 Years)', 6=>'PhD'] as $v=>$l)
                            <option value="{{ $v }}" {{ old('min_degree_level', $job->min_degree_level) == $v ? 'selected' : '' }}>{{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="experience_sector" class="form-select tom-select">
                            @foreach(['Any'=>'Any Sector Allowed', 'Public'=>'Public / Govt Only', 'Private'=>'Private Only'] as $val => $label)
                            <option value="{{ $val }}" {{ old('experience_sector', $job->experience_sector) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
            ...
            <div class="card-footer text-end">
                <a href="{{ route('admin.projects.show', $job->project_id) }}" class="btn btn-link">Cancel</a>
